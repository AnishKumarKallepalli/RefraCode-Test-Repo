...
def test_send_order_confirmation_invalid_email_raises():
    ...
    with pytest.raises(ValueError):
        send_order_confirmation(order_id='123', email='invalid-email', total=10.0)
...
def test_send_shipping_notification_short_tracking_number_raises():
    # Short tracking number raises ValueError
    with pytest.raises(ValueError):
        send_shipping_notification(tracking_number='ABC', email='test@example.com')
...
def test_send_shipping_notification_invalid_email_raises():
    # Invalid email format raises ValueError
    with pytest.raises(ValueError):
        send_shipping_notification(tracking_number='ABCD1234', email='invalid-email')